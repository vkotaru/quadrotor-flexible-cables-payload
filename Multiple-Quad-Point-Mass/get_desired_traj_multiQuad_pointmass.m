function[trajd]= get_desired_traj_multiQuad_pointmass(traj,data)
...
traj.T(3).T = (1/3)*mL*data.g*RotX(pi/6)*[0;0;1];
traj.T(2).T = (1/3)*mL*data.g*RotX(-pi/6)*[0;0;1];
traj.T(1).T = mL*traj.dx{2}-traj.T(3).T-traj.T(2).T + mL*data.g*data.e3;

for j = 1:(length(traj.dx)-2)
    traj.T(3).dT{j} = zeros(3,1);
    traj.T(2).dT{j} = zeros(3,1);
    traj.T(1).dT{j} = mL*traj.dx{j+2};
end
...
for i = 1:nQ
    ...
    trajd(i) = get_singleQuad_traj(traj,params,i);
...
function[trajectory] = get_singleQuad_traj(traj,params,quadNum)
...
B(n).B = traj.T(quadNum).T;
for i = 1:(imax-2)
    B(n).dB{i} = traj.T(quadNum).dT{i};
end
