function animate_3dmultiquad_flexcable(torig, xorig,params)
...
% aviobj = avifile('sample2.avi','compression','None');
    for i=1:length(t)
        ...
        axes1 = drawone(axes1, x(i,:,:),params);hold on;
        plot3(x(max(1,i-hist):i, 1,1), x(max(1,i-hist):i, 2,1), x(max(1,i-hist):i, 3,1), 'b') ;
...
function[parent]= drawone(parent, xd,params)
...
for j = 1:size(xd,3)   
    ...
    s(j).hload = plot3(xL(1,1), xL(2,1), xL(3,1), 'ko', 'LineWidth',lwl) ;
