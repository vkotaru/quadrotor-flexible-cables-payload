 function animate_3dquad_flexcable(torig, xorig,params,writeflag)
...
%     figure_z_limits = [-5 5] ;
    X = xorig;
    figure_x_limits = [-1+min(X(:,1)) 1+max(X(:,1))];
    figure_y_limits = [-1+min(X(:,2)) 1+max(X(:,2))];
    figure_z_limits = [-1+min(X(:,3)) 1+max(X(:,3))];
    ...
    set(fig1,'Position', [screen_x_offset screen_y_offset 1.75*screen_factor*figure_x_size 1.75*screen_factor*figure_y_size]);
...
MAKE_MOVIE = 0;
if writeflag
    aviobj = VideoWriter('planning1.avi','Uncompressed AVI'); 
    aviobj.FrameRate = 15;
    open(aviobj);
end
    ...
    for i=1:length(t)
        ...
        plot3(x(max(1,i-hist):i, 1), x(max(1,i-hist):i, 2), x(max(1,i-hist):i, 3), 'b') ;hold on;
