 function anime_mQpt(torig, xorig,xdesired,params,writeflag)
...
%==========================================
    ...
    set(fig1,'Position', [screen_x_offset screen_y_offset 1.5*screen_factor*figure_x_size 1.5*screen_factor*figure_y_size]);
...
%     xLdorig = xLd ;
    [t, x] = even_sample(torig, x, RATE);
     [~, xd] = even_sample(torig, xdesired, RATE);
    t = t+torig(1) ;
...
if writeflag
    aviobj = VideoWriter('3Q_2L_tensionCtrl.avi','Uncompressed AVI'); 
    aviobj.FrameRate = 15;
    open(aviobj);
end
    for i=1:length(t)
        ...
        plot3(xd(max(1,i-hist):i, 1), xd(max(1,i-hist):i, 2), xd(max(1,i-hist):i, 3), 'r','LineWidth',1) ;
...
function drawone(parent, x,data)
    ...
    xLoad = x(1:3);
    vLoad = x(4:6);
    states = mat2cell(x(7:end)',1,data.ns');   
for qNo = 1:data.nQ
    % Extracting parameters
    % =====================
    n = data.params(qNo).n-1; % No. of links
    m = data.params(qNo).m; % Mass of links
    l = data.params(qNo).l; % Length of links
    mQ = data.params(qNo).mQ; % Quad-rotor mass
    g = data.params(qNo).g ; 
    ...
    xL = x_(1:3);
    vL = x_(4:6);
    R = reshape(x_(7:15),3,3);
    Omega = x_(16:18);
    q = reshape(x_(19:3*n+18),3,n);
    omega = reshape(x_(3*n+19:6*n+18),3,n);
...
%     Omega = x(22:24) ;
    ...
    s.hload = plot3(xLoad(1,1), xLoad(2,1), xLoad(3,1), 'ks', 'LineWidth',lwl) ;
