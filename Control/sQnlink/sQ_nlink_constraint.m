...
addpath(genpath('~/kotaru/Quad_FlexCable/GeoControl-Toolbox'));
...
[time, P] = ode15s(@odefun_riccati, [25 0], xf, odeopts, data) ;
...
odeopts = odeset('RelTol', 1e-8, 'AbsTol', 1e-9) ;
% odeopts = [] ;
[t, x] = ode15s(@odefun_singleQuadFlex, [0 20], x0, odeopts, data) ;
...
for i = ind
   ...
   [C_] = get_linear_error_constraint(trajd,data);
   q_ = x(i,19:21)';  omega_ = x(i,22:24)';
   eq = cross(qd(:,1),q_);
   eom = omega_ - (-hat(q_)^2)*omegad;
   constriant = [constriant, C_*[eq; eom]];
...
figure;
subplot(1,2,1);
plot(t(ind),constriant(1,:),'r');
subplot(1,2,2);
plot(t(ind),constriant(2,:),'r');
...
function [C] = get_linear_error_constraint(trajd,data)
% Extracing parameters
% --------------------
% Dynamics of quadrotor suspended with load Constants
mQ = data.params.mQ;
...
if n > 1
    warning('current function is written only for one link');
end

C = [qd(:,1)', zeros(1,3);
        -omegad(:,1)'*hat(qd(:,1)), qd(:,1)'];
