...
data.params.m = [0.25]; % mass of each link
data.params.l = 1;% length of each link 
data.params.n = 1; % 
...
% 
xL0 = 0*trajd0.xL(data.params.n).x ;
...
for i = 1:n
    ...
    for j=1:n
        if i == j
            ...
            for k = 1:n
                if k~=i
                   temp = temp - Mij(i,k)*(hat(hat(qd(:,k))*dwd(:,k))+...
                       norm(wd(:,k))^2*hat(qd(:,k))); 
                end
            end
            F3(3*i-2:3*i,3*j-2:3*j) = temp*(-hat(qd(:,i)));
            F4(3*i-2:3*i,3*j-2:3*j) = O;
        else
            F3(3*i-2:3*i,3*j-2:3*j) = Mij(i,j)*hat(qd(:,i))*(hat(dwd(:,j))-...
                norm(wd(:,j))^2)*hat(qd(:,j));
