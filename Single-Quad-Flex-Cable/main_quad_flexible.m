function[varargout] =  main_quad_flexible(varargin)
% Equations of Motion for a Quadrotor UAV with a Payload Connected by
% Flexible Cable http://www.seas.gwu.edu/~tylee/paper/ACC14.1.pdf
% FEED-Forward Simulation with inputs calculated using differential-flatness
% 
...
% data.params.m = [0.01,0.01,0.01,0.01,0.1]; % 0.1*ones(1,5); % mass of each link
% data.params.l = 0.25*ones(1,5); % length of each link 
% data.params.n = 5; % No. of links suspended

% data.params.m = [0.0133,0.0133,0.0133,0.1]; % 0.1*ones(1,5); % mass of each link
% data.params.l = 0.3125*ones(1,4); % length of each link 
% data.params.n = 4; % No. of links suspended

% data.params.m = [0.02,0.02,0.1]; % 0.1*ones(1,5); % mass of each link
% data.params.l = 0.4167*ones(1,3); % length of each link 
% data.params.n = 3; % No. of links suspended

% data.params.m = [0.04,0.1]; % 0.1*ones(1,5); % mass of each link
...
[trajd0] = get_desired_traj(get_flat_traj_10link(0,freq),data.params);
...
% odeopts = [] ;
[t, x] = ode15s(@odefun_singleQuadFlex, [0 15], x0, odeopts, data) ;
...
if nargout > 0
    output.x = x;
    output.xd = xd;
    output.psi_exL = psi_exL;
    output.psi_evL = psi_evL;
    output.data = data;
    output.freq = freq;
    output.f = f;
    output.M = M;
...
[trajd] = get_desired_traj(get_flat_traj_10link(t,freq),data.params);
% inputs
f = trajd.f;
M = trajd.M;
% f = M00*g;
